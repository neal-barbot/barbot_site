...
function buildDiagramPrompt(description: string, language: string): { system: string; user: string } {
  ...
  const user = [
    ...
    '',
    '绘图规范（按题材自适应）：',
    '- 电子系统 → EE 惯例：信号流从左到右，电源输入/接口在最左，主控居中，执行/输出/通信在右；',
    '  edge 标注电压轨（"5V"、"3.3V"）或接口协议（SPI / I2C / CAN / UART / isoSPI）；反馈/使能/告警用 dashed: true',
    '- 机器/工业流程 → 按真实物料路径逐级画（如水路、咖啡路），edge 标注流转的物料或状态',
    '- 科学/生物等概念 → 按该领域的标准阶段顺序画，edge 标注传递的分子/能量/物质；',
    '  除非题材本身是电子系统，否则禁止杜撰 MCU、电源、传感器等电子模块',
    '- 每个 block 是一个功能单元/阶段，label 用真实名称，sub 一句话说明',
    '- group 用中文功能域名称，同域 block 用相同 group',
    '- block 数量控制在 6–18 个，抓主干，不画旁路细节',
