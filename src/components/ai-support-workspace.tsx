...
function HumanSupportPanel({ chatbotId }: { chatbotId: string }) { const queryClient = useQueryClient(); const query = useQuery({ queryKey: ['human-support', chatbotId], queryFn: () => apiGet<Record<string, unknown>>(`/api/ai-support/human-support-settings?chatbotId=${chatbotId}`) }); const [value, setValue] = useState(''); const current = value || JSON.stringify(query.data ?? {}, null, 2); const mutation = useMutation({ mutationFn: () => apiPatch('/api/ai-support/human-support-settings', { chatbotId, settings: JSON.parse(current) }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['human-support', chatbotId] }); setValue(''); toast.success('Published human support settings'); } }); return <Card><CardContent className="space-y-3 pt-6"><Textarea className="min-h-80 font-mono text-xs" value={current} onChange={(event) => setValue(event.target.value)} /><Button onClick={() => mutation.mutate()}><Save className="size-4" />Publish</Button></CardContent></Card>; }
